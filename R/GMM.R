...
gGMMThresh <- function(gerris_crops, nG=2, noise_n=0.005, y_root=-25, msg=TRUE){
  ...
  l_GMM <- GMM(img_val = g_val_clean, n = nG,
               keep_max = FALSE, n_noise_sd = noise_n) #Fit two weighted gaussians to slighlty noised data
...
GMM <- function(img_val, n=1:9, keep_max=TRUE, viz=FALSE, n_noise_sd=0){
  ...
  mod <- Mclust(img_val, modelNames="V", G=n, verbose=FALSE) #fit model on cropped removed_body noised data
  ...
  if(viz){
    hist(img_val, breaks=500, freq=FALSE, border=NA) #sum classes = 1
...
#' @param viz visualization option
#' @export
GMMquant <- function(gmm_model_df, q, viz=FALSE){
  f <- function(x){
...
#' @export
GMMdRoots <- function(GMM_df, y, viz=FALSE, msg=TRUE){
