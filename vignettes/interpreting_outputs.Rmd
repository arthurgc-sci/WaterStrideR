...
WaterStrideR produces three types of outputs to help you with extracting 
measurements and validating your results:

1. **R data frame** - Measurement data returned directly in R
2. **Folder outputs** - CSV file and graphical plots for quality control
3. **Advanced outputs** - Intermediate processing data for in depth analysis

This vignette explains how to interpret each output type and how to use them for
quality control.
...
df_demo <- results$df
df_demo #Explore the data frame
```
...
- `L_body` - Body length (along the detected antero-posterior axis)
- `error_margin` - If `auto_scale = TRUE`: error margin on body length linked to scale computation
- `left_tibia`, `right_tibia` - Hind leg's tibia measurements
- `left_femur`, `right_femur` - Hind leg's femur measurements
...
- **NA values:** Indicate failed measurements (failed safety check during either segmentation or landmarking)
...
**⚠️ Important:** These predictions are reliable only for images of *M. longipes* 
acquired using the same acquisition protocol as described herein. Low confidence values 
(<0.90) should encourage you to check manually.

**Note:** In the current configuration, of you want sex and wing status prediction, 
running the entire pipeline is required regardless of your interest in leg measurement.
...
- Link measurements to corresponding plots
...
- Small white crosses: individuals centroids (center of gravity)
- Light blue numbers: Individual IDs (matching the `i` column of the data frame)
...
- **Troubleshooting:** May be used for tuning the parameters of scale detection and individual detection.
...
- **Pink arrow:** Body orientation from body centroid (to the anterior direction)
...
- **Validate segmentation quality:** Are body and legs properly segmented?
- **Check orientation accuracy:** Does pink arrow point to the anterior?
...
**Intermediate outputs include (but are not limited to):**

-`body_img`: Segmented and cropped body image
-`angle`: Orientation angle
-`legs`: Hind legs points
-`dilcont`: Dilated contour (sorted list of pixels)
...
-`scale`: Scaling factor, and error margin if `auto_scale = TRUE`

### Use Cases

These outputs were not designed with a specific purpose in mind. We chose to include them for multiple reasons:

- **Extraction of additional information** not deemed essential for inclusion in the main results data frame, such as the scaling factor, orientation, or body centroid. These variables may be useful for specific applications but are excluded from the default output to avoid unnecessary bloat.
